...
public class PlayerMovement : NetworkBehaviour
{
    ...
    [SerializeField]
    /**
     * 由于游戏开发后期需要对人物角色施加力（例如抓钩，蹭墙），所以改用rigidbody组件
     */
    ...
    public bool OnSlope()
    {
        if (Physics.Raycast(transform.position, Vector3.down, out slopeHit, groundedOffset * 0.5f + 0.3f))
        {
    ...
    //     Vector3 flatVelocity = new Vector3(_rigidbody.velocity.x,0f,_rigidbody.velocity.z);
    //     
    //     float currentModeSpeed= allowSprinting && _sprint ? sprintSpeed : moveSpeed;
    //
    //     if (flatVelocity.magnitude > currentModeSpeed)
    //     {
    //         //Debug.LogWarning("over speed");
    //         Vector3 limitVelocity = flatVelocity.normalized * currentModeSpeed;
    //         _rigidbody.velocity = new Vector3(limitVelocity.x, _rigidbody.velocity.y, limitVelocity.z);
    //     }
    // }
